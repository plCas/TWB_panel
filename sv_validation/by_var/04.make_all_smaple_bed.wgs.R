...
#### input files ####
DIR_TP_wgs = "/../by_var/bed_files/wgs" # directory of bed files generate from "00.2.intersect_imp_wgs.sh" 
FILE_imp_sample_list="/../by_var/vcfs/imp_sample_list"
FILE_imp_bed_position_only = "/../by_var/bed_files/all_SV_imp_position-only.bed" # pre generate position only bed file from "all_SV_imp.bed"
...
# imputed SVs bed
bed_imp = read.table(FILE_imp_bed_position_only, header = T)
...
bed_imp = bed_imp[rowSums(!is.na(bed_imp[,5:ncol(bed_imp)]))>0,] # genotypes are started from column 5
